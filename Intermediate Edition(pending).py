...
#The first part: Consumer
    ...
    def utility(self,input):
        ...
        # utility of the goods consumed in a array form
         ...
         goods_utility = np.empty(self.nc * self.cgl)
    ...
    #Return the quantity demand
    def quantity_demand(self,w,r,p,MaxUtility):
    ...
    def production(self,input, w, r):
         ...
         if self.externality == False:
          # Production of goods under no externality in an economy
           ...
           def maximizedoutput_noe(self,w,r):
                ...
                maximized_output_noe = np.empty(self.npro)
                #Fill this array this each firm's maximized profit via F.O.C
             
            for j in self.production_noexternality:
                # y = MPL*l + MPK *K
                maximized_output_noe[i] =((diff(total_profit_noe[i],w))*factors_labor[i] + (diff(total_profit_noe[i],r)*factors_capital[i] )
        ...
        #Profit array under externlaity 
            ...
            for j in self.production_externality:
                ...
                def maximizedoutput_externality(self,w,r):
                ...
                #Fill this array this each firm's maximized profit via F.O.C
                     for j in self.production_externality:
                # y = MPL*l + MPK *K
                         maximized_output_e[i] =((diff(total_profit_e[i],w))*factors_labor[i] + (diff(total_profit_e[i],r)*factors_capital[i]))
                 #The maximized profit array 
                          return maximized_output_e
        
                def  maximized_factors_e(self,w,r,maximizedoutput_e):
                     total_factors_labor_e = np.empty(self.npro)
                     total_factors_capital_e = np.empty(self.npro)  
                 
                     for l in self.maximizedoutput_e:
                          total_factors_labor_e[i] = (maximized_output_e[i]-(diff(total_profit_e[i],r)*factors_capital[i] ))/((diff(total_profit_e[i],w)))
                          total_factors_capital_e[i] = (maximized_output_e[i]-(diff(total_profit_e[i],w)*factors_labor[i] ))/((diff(total_profit_e[i],r)))
                          return total_factors_labor_e
                          return total_factors_capital_e
...
#The 3 market equilibrium without externality  
if self.externality = False:
